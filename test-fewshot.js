...
// Test articles (same as before for comparison)
const TEST_ARTICLES = [
  // SHOULD BE RELEVANT
  {
...
async function runTest() {
  console.log("🧪 Testing Jina FEW-SHOT Classifier\n");
  ...
  try {
    ...
    results.forEach((result, index) => {
      ...
      console.log(`${icon} ${status} (score: ${result.score.toFixed(3)}) - Expected: ${testArticle.expectedRelevant ? "RELEVANT" : "NOT RELEVANT"}`);
    ...
    if (relevantScores.length > 0 && irrelevantScores.length > 0) {
      ...
      console.log(`(Higher is better - indicates clearer distinction)`);
    }

    console.log("\n" + "=".repeat(80));
    console.log("\n🆚 COMPARISON TO ZERO-SHOT:");
    console.log("   Zero-shot accuracy: 60%");
    console.log(`   Few-shot accuracy: ${accuracy}%`);
    console.log(`   Improvement: ${(parseFloat(accuracy) - 60).toFixed(1)}%`);

    if (accuracy >= 90) {
      console.log("\n✅ EXCELLENT: Few-shot training worked great!");
    } else if (accuracy >= 70) {
      console.log("\n⚠️  GOOD: Better than zero-shot but room for improvement.");
    } else {
      console.log("\n❌ POOR: Not much better than zero-shot.");
    }
